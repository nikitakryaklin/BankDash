    ...
    const result = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "useWeeklyActivityCalculator.useMemo[result]": ()=>{
            ...
            const sorted = [
                ...
                "useWeeklyActivityCalculator.useMemo[result].sorted": ([a], [b])=>a.localeCompare(b)
            }["useWeeklyActivityCalculator.useMemo[result].sorted"]);
            const labels = sorted.map({
                "useWeeklyActivityCalculator.useMemo[result].labels": ([, el])=>el.day.slice(0, 3)
            }["useWeeklyActivityCalculator.useMemo[result].labels"]);
            const debit = sorted.map({
                "useWeeklyActivityCalculator.useMemo[result].debit": ([, el])=>el.data.expenses
            }["useWeeklyActivityCalculator.useMemo[result].debit"]);
            const replenishment = sorted.map({
                "useWeeklyActivityCalculator.useMemo[result].replenishment": ([, el])=>el.data.replenishment
            }["useWeeklyActivityCalculator.useMemo[result].replenishment"]);
            return {
                isTitle: false,
                isLegend: true,
                isGrid: true,
...
_s(useWeeklyActivityCalculator, "x7P70xJwHfQqwOXrAZBzywOUBX0=", false, function() {
    return [
...
//# sourceMappingURL=src_5d95032d._.js.map